...
import { listCreatorPages } from 'services/creator.service';
...
export default function PageList() {
  ...
  const { isLoading, data, fetchNextPage, hasNextPage } = useInfiniteQuery({
    queryKey: [`creators-${sub}`, { CreatorOwnerID: pvtPublicQuery }],
    queryFn: listCreatorPages,
    getNextPageParam: (lastPage) => lastPage?.creators?.nextToken,
  });
    ...
    <>
      <div className="px-4 py-6 c-container">
        ...
        <section className="grid grid-cols-1 gap-5 py-6 pb-20 gap-y-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          ...
          {!isLoading && !data?.pages?.[0].creators?.items?.length && (
            <div className="col-span-full">
              <NoSearchResults
                filled={false}
                label="You don't have any Creator pages yet!"
              />
            </div>
          )}
          {!isLoading &&
            data?.pages?.map((i) => (
              <>
                {i?.creators?.items?.map((pageDetail) => {
                  return (
                    <PageCard
