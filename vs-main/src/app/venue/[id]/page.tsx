import VenueProfile from 'src/venue';
...
export async function generateMetadata({
  ...
  try {
    const res = await virtuosoServerUnauth.get(
      `/api/public/venue/metadata/${params.id}`
    );
    const page = res?.data?.data;
    const url = `https://app.virtuoso.live/venue/${page?.id || params.id}`;

    return {
      title: `${page?.title} - Venue Page`,
...
export default async function CreatorPage({ params }: { params: any }) {
  ...
  return <VenueProfile data={response?.data?.data} />;
}
