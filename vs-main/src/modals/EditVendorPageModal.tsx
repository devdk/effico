...
import { updateVendorHeader } from 'services/vendor.service';
...
type ModalProps = {
  isOpen: boolean;
  data: VirtuosoVendors;
  setIsOpen: (e: boolean) => void;
};

const EditVendorPageModal = ({
  isOpen,
  setIsOpen,
  data: vendor,
}: ModalProps) => {
  ...
  const [image, setImage] = useState(vendor?.VendorImage || '');
  const [cover, setCover] = useState(vendor?.VendorCover || '');
  const [name, setName] = useState(vendor?.VendorName || '');
  const [bio, setBio] = useState(vendor.VendorBio || '');
  ...
  const onSuccess = (data: UpdateVendorHeaderMutation) => {
    queryClient.setQueryData(
      ['vendor', data?.update?.VendorID, session?.user?.sub],
      (old: any) => {
        // console.log({ old });
        return {
          vendor: {
  ...
  const { mutate, isLoading } = useMutation<
    ...
    UpdateVendorHeaderMutationVariables
  >(updateVendorHeader, {
    mutationKey: 'update-vendor-header',
    onSuccess,
    onError,
  });
...
export default EditVendorPageModal;
